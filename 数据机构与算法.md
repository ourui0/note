...
```
=======
>>>>>>> 971c9644cd3a17d4d9c7af4b928b32cf82fcdb8c

## 最小生成树

### 核心定义

**最小生成树**是一个加权无向图的生成树，并且这棵树上所有边的权重总和是**最小**的

### 什么是“生成树”

想象一下，有几个城市和连接他们的道路

**1.树：**首先，“树”是一种特殊的图，他必须满足两个条件：

+ **连通：**任意两个城市之间都有路径可以到达。
+ **无环：**从任何一个城市出发，都不存在一条能不重复地回到自身的环路

**2.生成：**意思是这棵树要“覆盖”原图的所有顶点（城市）。也就是说，生成树必须包含原图的所有顶点，但只保留足够联通这些顶点的边。

**简单来说，生成树就是：用最少的便，把一个图的所有顶点连接起来，并且保证没有环路。**


### 什么是“最小”？

现在，我们给地图上的每条道路加上一个“成本”，比如修路的费用、或者距离。这个成本就是边的**权重**。

+ **最小**指的就是，在所有可能的生成树中，我们选择**所有边的权重之和最小**的那一棵

**所以，最小生成树就是：用总成本最低的方式，把所有城市连接起来，并且保证道路网络没有环**。

### 如何寻找最小生成树？

#### Kruskal算法（加边法）

+ **思想：**从小到大考虑边的权重，每次选择权重最小的边，如果加入这条边不会与已选择的边形成环路，就把他划入生成树中。
>>>>>>> b14890a252d6091ccfe9b0799330e09368675de0
